...
const double meanOnTime = 5.0;
const double meanOffTime = 2.0;
const double ackSize = 50;
const double congestionRate = 10e6;
const double congestionDuration = 0.1;
const double serverDelayMean = 0.001;
const double computePerKb = 0.01;

std::default_random_engine generator(1234);
std::ofstream csvFile;
std::vector<int> ueTrafficType;
std::vector<double> sliceBandwidth = {0.5, 0.3, 0.2}; // eMBB, URLLC, mMTC
std::vector<double> sliceCpuLimit = {100.0, 50.0, 200.0};
std::vector<double> sliceMemoryLimit = {100.0, 50.0, 200.0};

// Initialize Python interpreter
PYBIND11_EMBEDDED_MODULE(ns3_rl, m) {
    m.def("choose_action", [](int state, const std::vector<double>& traffic_stats, double server_cpu, double server_memory) {
        py::gil_scoped_acquire acquire;
        py::module_ rl = py::module_::import("rl_model");
        py::object result = rl.attr("choose_action")(state, traffic_stats, server_cpu, server_memory);
                         ...
                         NodeContainer ues, NodeContainer routers, Ptr<Node> server)
{
    ...
    py::gil_scoped_acquire acquire;
    std::vector<double> newBandwidth = py::module_::import("ns3_rl").attr("choose_action")(
        state, std::vector<double>{trafficStats[0], trafficStats[1], trafficStats[2]}, serverCpu, serverMemory
    ).cast<std::vector<double>>();

    // Validate and apply bandwidth
    if (newBandwidth.size() == 3 && std::abs(newBandwidth[0] + newBandwidth[1] + newBandwidth[2] - 1.0) < 0.01)
    {
        sliceBandwidth = newBandwidth;
    }
    else
    {
        sliceBandwidth = {0.33, 0.33, 0.34}; // Fallback
    }

    // Log state and bandwidth
    csvFile << Simulator::Now().GetSeconds() << ",STATE,-1,-1,-1,-1,-1,-1,-1,0,-1,"
...
int main(int argc, char *argv[])
{
    ...
    {
        int sliceId = ueTrafficType[i];
        if (sliceId == 0) anim.UpdateNodeColor(ues.Get(i), 255, 0, 0);
        else if (sliceId == 1) anim.UpdateNodeColor(ues.Get(i), 0, 255, 0);
