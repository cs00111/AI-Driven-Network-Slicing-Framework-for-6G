...
public:
    VirtualSlice(std::string name, double bandwidth, uint32_t maxQueue)
        : name(name), bandwidthMbps(bandwidth), maxQueueBytes(maxQueue), packetsDropped(0), packetsEnqueued(0),
          totalLatency(0.0), totalBytes(0.0), packetRate(0.0) {}
...
public:
    ...
    bool ProcessPacket(Ptr<Packet> packet, Time arrivalTime, double packetRate)
    {
        ...
        LatencyTag tag;
        packet->PeekPacketTag(tag);
        double latency = (Simulator::Now() - arrivalTime).GetSeconds();
        slices[slice].totalLatency += latency;
        slices[slice].packetRate = packetRate;
        
        csvFile << Simulator::Now().GetSeconds() << "," << packet->GetUid() << "," << packet->GetSize() << ","
                << tag.GetLatency() << "," << packetRate << "," << slices[slice].name << "," << enqueued << ","
    ...
    void MonitorAndAdjust()
    {
        ...
        if (std::abs(totalBw - totalBandwidth) > 0.01)
            NS_LOG_WARN("Total bandwidth mismatch: " << totalBw << " != " << totalBandwidth);
        ...
        for (size_t i = 0; i < slices.size(); ++i)
        {
            ...
            if (dropRate > dropThreshold || avgLatency > latencyThresholds[i])
            {
                ...
                if (slices[donor].bandwidthMbps > extraBw + 10.0)
                {
...
private:
    ...
    void HandleReceive(Ptr<Socket> socket)
    {
        ...
        while ((packet = socket->RecvFrom(from)))
        {
            ...
            double packetRate = 0.0; // Placeholder; ideally passed from UE
            m_controller->ProcessPacket(packet, Simulator::Now(), packetRate);
        }
    }
...
int main(int argc, char *argv[])
{
    ...
    csvFile << "Time,Packet_ID,PacketSize,LatencyReq,PacketRate,Slice,Enqueued,Dropped,Slice_BW,Queue_Occupancy\n";
    ...
    // Log UE device creation instead of GetCellId
